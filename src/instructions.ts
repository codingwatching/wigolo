...
Prefer mode="structured" over chaining multiple extract calls — one response carries \`{ tables, definitions, jsonld, chart_hints, key_value_pairs }\`. chart_hints surfaces SVG titles, aria-labels, figcaptions for charts whose data is JS-rendered. Metadata parity with \`fetch\` (same og_/canonical_url shape). \`mode: "brand"\` walks JSON-LD Organization/Brand/WebSite → OG/Twitter Card meta → \`<link rel=icon>\` → CSS custom properties (\`--brand-primary\`, \`--color-primary\`) → heuristic header/footer DOM; \`provenance\` records the winning source for logo/colors/fonts so callers can trust the values. \`provenance\` enums: logo ∈ {json-ld, og:logo, link[rel=icon], heuristic, unknown}; colors ∈ {css-vars, palette-extraction, unknown}; fonts ∈ {css-vars, css-rule, inline-style, google-fonts-link, unknown}. Honesty contract: \`name\` is unset when no explicit name source emits one (the <title> is not a name source); \`logo_url\` is unset when no real logo source emits one (favicons never promote to \`logo_url\` — they stay in \`favicon_url\`). \`mode: "schema"\` enforces evidence-only: every LLM-sourced field is verified against source text; hallucinated values are returned as \`null\` with a warning.`,
